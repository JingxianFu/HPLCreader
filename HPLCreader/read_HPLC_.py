...
def read_web(f_input):
    content_dict = {}
    content = f_input['content'].decode('utf-8').replace('\r', '')
...
def find_character(df, time=23.021, Threshold=0.25):
    time_df = df
    char = time_df[abs(time_df['RT.time']-time)<Threshold]['Area'].values
    if len(char) > 1:
        char = max(char)
    else:
        char = char[0]
